...
# ACM Certificate
resource "aws_acm_certificate" "statuspage_cert" {
  ...
  tags = {
    Name = "${var.prefix}statuspage-certificate"
  }
}
...
resource "aws_route53_record" "statuspage_cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.statuspage_cert.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "statuspage_cert" {
  certificate_arn         = aws_acm_certificate.statuspage_cert.arn
  validation_record_fqdns = [for record in aws_route53_record.statuspage_cert_validation : record.fqdn]
}
